...
export function useRealtimeSolicitations() {
  ...
  const deleteSolicitation = useCallback(async (id: string): Promise<boolean> => {
    console.log('🗑️ [DELETE] Iniciando exclusão da solicitação:', id);
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      let pdfUrl = null;
      console.log('🔍 [DELETE] Buscando dados da solicitação para exclusão...');
      ...
      try {
        ...
        console.log('📋 [DELETE] Dados da solicitação encontrada:', {
          id,
          nome: solicitation?.nome,
          matricula: solicitation?.matricula,
        ...
        if (!fetchError && solicitation?.pdf_laudo) {
          ...
          console.log('📄 [DELETE] PDF encontrado, será deletado do storage:', pdfUrl);
        } else {
          console.log('📄 [DELETE] Nenhum PDF encontrado ou erro na busca');
        }
      } catch (fetchError) {
        console.log('⚠️ [DELETE] Erro ao buscar dados da solicitação:', fetchError);
        // Continua a operação mesmo se não conseguir buscar o PDF
      }

      // Deletar a solicitação do banco
      console.log('🗑️ [DELETE] Executando DELETE no banco de dados...');
      ...
      console.log('🗑️ [DELETE] Resultado do DELETE:', {
        deleteData,
        error,
      ...
      if (error) {
        console.error('❌ [DELETE] Erro ao deletar do banco:', error);
        throw error;
      }

      if (!deleteData || deleteData.length === 0) {
        console.warn('⚠️ [DELETE] Nenhuma linha foi afetada pelo DELETE');
        throw new Error('Nenhuma solicitação foi encontrada para exclusão');
      }

      console.log('✅ [DELETE] Solicitação deletada do banco com sucesso');

      // Se existe PDF, deletar do storage
      if (pdfUrl) {
        console.log('🗂️ [DELETE] Deletando PDF do storage...');
        try {
          const { deletePDFFromStorage } = await import('@/lib/supabase-storage');
          const storageResult = await deletePDFFromStorage(pdfUrl);
          console.log('🗂️ [DELETE] Resultado da exclusão do storage:', storageResult);
        } catch (storageError) {
          console.error('❌ [DELETE] Erro ao deletar PDF do storage:', storageError);
          // Não falha a operação se não conseguir deletar o PDF
        }
      } else {
        console.log('🗂️ [DELETE] Nenhum PDF para deletar do storage');
      }
      ...
      setSolicitations(prev => {
        ...
        console.log('🔄 [DELETE] Lista atualizada:', {
          antes: prev.length,
          depois: updated.length,
          removido: prev.length - updated.length
        });
        return updated;
      });
      ...
      console.error('❌ [DELETE] Erro na exclusão:', {
        error: err,
      ...
      console.log('🏁 [DELETE] Finalizando processo de exclusão');
      setLoading(false);
    }
  }, []);
